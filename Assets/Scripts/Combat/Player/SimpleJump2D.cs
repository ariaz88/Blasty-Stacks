...
public class FrogJumpTransformOnly : MonoBehaviour
{
    ...
    private void BeginJump1()
    {
        ...
        // switch to loop a bit after start, before apex
...
public class FrogJumpTransformOnly2 : MonoBehaviour
{
    [Header("Refs")]
    [SerializeField] private Animator animator;             // optional
    [SerializeField] private SpriteRenderer spriteRenderer; // optional
    ...
    [Header("Jump Shape (transform-only)")]
    [Tooltip("Total forward distance along Y you will travel during the jump (+up / -down).")]
    [SerializeField] private float jumpDistanceY = 3.0f;
    [Tooltip("Maximum arc height added during the jump (visual lift).")]
    [SerializeField] private float arcHeight = 1.5f;
    [Tooltip("Seconds it takes to complete the jump from start to land.")]
    [SerializeField, Min(0.05f)] private float jumpDuration = 0.45f;

    [Header("Timing")]
    [Tooltip("Delay before swapping JumpStart → JumpLoop.")]
    [SerializeField] private float jumpStartToLoopDelay = 0.12f;

    [Header("Facing on Y")]
    [Tooltip("If true uses SpriteRenderer.flipY; else uses transform.localScale.y sign.")]
    [SerializeField] private bool useFlipYFacing = false;

    [Header("QoL (buffer)")]
    [SerializeField] private float jumpBuffer = 0.12f;

    [Header("Air Scale Effect")]
    [Tooltip("Uniform scale at the very apex (1 = no change, 0.85 = 15% smaller).")]
    [SerializeField, Range(1f, 1.50f)] private float apexScale = 1.2f;
    [Tooltip("How quickly scaling follows the arc (1 = match arc, >1 = stronger near apex).")]
    [SerializeField, Min(0.1f)] private float scaleResponse = 1.0f;
    [Tooltip("Return instantly to normal on land (true) or keep last in-air scale (false).")]
    [SerializeField] private bool resetScaleOnLand = true;

    // Arc profile: t in [0,1] -> height multiplier (0 at start & end, 1 at apex)
    private float HeightCurve(float t) => 4f * t * (1f - t);
    ...
    private float tElapsed;       // time since jump started
    private float tLoopSwapAt;    // time to switch to loop

    // scale bookkeeping
    private Vector3 baseScaleAbs;   // absolute original scale magnitudes
    private Vector3 baseScaleSign;  // sign of original scale (to preserve flips)

    private void Awake()
    {
        var s = transform.localScale;
        baseScaleAbs = new Vector3(Mathf.Abs(s.x), Mathf.Abs(s.y), Mathf.Abs(s.z));
        baseScaleSign = new Vector3(Mathf.Sign(s.x), Mathf.Sign(s.y), Mathf.Sign(s.z));
        if (baseScaleSign.x == 0) baseScaleSign.x = 1;
        if (baseScaleSign.y == 0) baseScaleSign.y = 1;
        if (baseScaleSign.z == 0) baseScaleSign.z = 1;
    }
    ...
    private void Update()
    {
        ...
        // Start a new jump only if not already jumping
        if (!isJumping && hasBufferedJump)
            BeginJump();

        // --- Animate current jump (parametric arc) ---
        if (!isJumping) return;

        tElapsed += Time.deltaTime;
        float t = Mathf.Clamp01(tElapsed / jumpDuration);

        // Forward along Y from start to end
        float yLinear = Mathf.Lerp(startPos.y, endPos.y, t);

        // Add arc height (visual lift) without changing final Y
        float arcT = HeightCurve(t);
        float yWithArc = yLinear + arcT * arcHeight;
        ...
        // ---- Air scaling toward apex ----
        // Map arcT (0..1) -> scale factor (1..apexScale)
        // Optionally accentuate with a power (scaleResponse)
        float arcForScale = Mathf.Pow(arcT, scaleResponse);
        float scaleU = Mathf.Lerp(1f, apexScale, arcForScale); // 1 at takeoff/land, apexScale at apex

        Vector3 scaledAbs = baseScaleAbs * scaleU;
        transform.localScale = new Vector3(
            scaledAbs.x * baseScaleSign.x,
            scaledAbs.y * baseScaleSign.y,
            scaledAbs.z * baseScaleSign.z
        ...
        // Swap to loop after a short delay (while still airborne)
        if (!loopPlayed && tElapsed >= tLoopSwapAt && t < 1f)
        {
            PlayAnim(jumpLoopState);
            loopPlayed = true;
        }
    ...
    private void BeginJump()
    {
        ...
        isJumping = true;
        loopPlayed = false;
        tLoopSwapAt = Mathf.Min(jumpStartToLoopDelay, jumpDuration * 0.5f); // swap before apex
...
public class FrogJumpTransformOnly1 : MonoBehaviour
{
    [Header("Refs")]
    [SerializeField] private Animator animator;             // optional
    [SerializeField] private SpriteRenderer spriteRenderer; // optional
    ...
    // Arc profile: t in [0,1] -> height multiplier (0 at start & end, 1 at apex)
    // You can tweak points in the Inspector if you expose it, but a simple bell curve works:
    private float HeightCurve(float t)
    {
        // Smooth bell (0→1→0): 4t(1−t)
        return 4f * t * (1f - t);
    }
    ...
    private void Update()
    {
        ...
        if (isJumping)
        {
            ...
            // Add arc height (visual lift) without changing final landing Y
            float yWithArc = yLinear + HeightCurve(t) * arcHeight;
            ...
            if (t >= 1f)
            {
                // Hard snap to end Y to avoid any float drift
                Vector3 landed = transform.position;
                landed.y = endPos.y;
                transform.position = landed;
    ...
    private void BeginJump()
    {
        ...
        isJumping = true;
        loopPlayed = false;
        tLoopSwapAt = Mathf.Min(jumpStartToLoopDelay, jumpDuration * 0.5f); // safe swap before apex
...
// Jump in y direction
public class FrogJump2D2 : MonoBehaviour
{
    [Header("Refs")]
    ...
    [SerializeField] private string jumpStartState = "JumpStart";
    [SerializeField] private string jumpLoopState = "JumpLoop";
    [SerializeField, Min(0f)] private float jumpStartToLoopDelay = 0.15f;

    [Header("Jump Forces")]
    [Tooltip("Base upward impulse on Y (the 'jump').")]
    [SerializeField, Min(0f)] private float verticalImpulse = 12f;
    [Tooltip("Extra forward push ON THE Y AXIS (frog-style).")]
    [SerializeField, Min(0f)] private float forwardImpulseY = 6f;
    ...
    [SerializeField, Min(0f)] private float groundRadius = 0.1f;
    ...
    [Header("Facing on Y")]
    [Tooltip("If true, uses SpriteRenderer.flipY for up/down facing; otherwise uses transform.localScale.y sign.")]
    [SerializeField] private bool useFlipYFacing = false;
    ...
    private float lastJumpPressedTime = -999f;   // prevents auto-jump at start
    private float jumpStartPlayedAt;
    ...
    private void Update()
    {
        ...
        if (!isJumping && hasBufferedJump && (isGrounded || canCoyote))
        {
            DoForwardJumpY();
        }

        // Swap JumpStart -> JumpLoop
        if (isJumping && !loopPlayed && Time.time - jumpStartPlayedAt >= jumpStartToLoopDelay)
        {
            if (!isGrounded || rb.linearVelocity.y > 0.01f)
            {
                if (animator && !string.IsNullOrEmpty(jumpLoopState))
                    animator.CrossFadeInFixedTime(jumpLoopState, 0.05f);
                loopPlayed = true;
            }
        }
    }

    private void DoForwardJumpY()
    {
        // consume buffer
        lastJumpPressedTime = -999f;

        // Determine up/down facing (+1 = up, -1 = down)
        int facingY = GetFacingYSign();

        // Reset vertical velocity for consistent jump height
        rb.linearVelocity = new Vector2(rb.linearVelocity.x, 0f);

        // Apply impulse ONLY on Y (base jump + forward Y push)
        float yImpulse = verticalImpulse + forwardImpulseY * facingY;
        Vector2 impulse = new Vector2(0f, yImpulse);
        rb.AddForce(impulse, ForceMode2D.Impulse);

        // Play JumpStart
        if (animator && !string.IsNullOrEmpty(jumpStartState))
            animator.CrossFadeInFixedTime(jumpStartState, 0.05f);

        isJumping = true;
        loopPlayed = false;
        jumpStartPlayedAt = Time.time;
    }
...
// Jumpimng in x direction
public class FrogJump2D1 : MonoBehaviour
{
    [Header("Refs")]
    [SerializeField] private Animator animator;
    [SerializeField] private Rigidbody2D rb;
    [SerializeField] private SpriteRenderer spriteRenderer; // optional (only if you want flipX)
    ...
    [Header("Jump Forces")]
    [SerializeField, Min(0f)] private float verticalImpulse = 12f;
    [SerializeField, Min(0f)] private float forwardImpulse = 6f;  // << add forward power here
    ...
    [Tooltip("If true, use SpriteRenderer.flipX; otherwise use transform.localScale.x")]
    [SerializeField] private bool useFlipXFacing = false;
    ...
    private bool loopPlayed;
    private float lastGroundedTime;
    private float lastJumpPressedTime = -999f;   // << prevents auto-jump on scene start
    private float jumpStartPlayedAt;
    ...
    private void DoForwardJump()
    {
        ...
        // Determine facing (+1 = right, -1 = left)
        int facing = GetFacingSign();

        // Reset vertical velocity for consistent jump height
        rb.linearVelocity = new Vector2(rb.linearVelocity.x, 0f);

        // Apply frog-style impulse (forward + up)
        Vector2 impulse = new Vector2(forwardImpulse * facing, verticalImpulse);
        rb.AddForce(impulse, ForceMode2D.Impulse);
    ...
    {
        if (useFlipXFacing && spriteRenderer != null)
            return spriteRenderer.flipX ? -1 : 1;

        float sx = transform.localScale.x;
        if (Mathf.Approximately(sx, 0f)) return 1;
        return sx > 0f ? 1 : -1;
    }
