...
public class EnemyLocoMotion : MonoBehaviour
{
    ...
    public void HandleMoveToTarget()
    {
        ...
        if (currentTarget != null && !currentTarget.playerIsdead)
        {
            ...
            //if (dist > stoppingDistance)
            //{
            //    // lane-based: move in facing direction, not steering to the player
            ...
            //else
            //{
            //    // in melee range -> stop moving, EnemyManager will handle attacks
            //    enemyRigidbody2D.bodyType = RigidbodyType2D.Kinematic;
            //    SetAnimMoving(false);
            //}
...
public class EnemyLocoMotion1 : MonoBehaviour
{
    EnemyManager enemyManager;
    [Header("Refs")]
    public Rigidbody2D enemyRigidbody2D;            // 2D body
    ...
    void Awake()
    {
        enemyManager = GetComponent<EnemyManager>();
        //enemyManager = GetComponent<EnemyManager>();
        //if (!enemyAnimatorManager) enemyAnimatorManager = GetComponentInChildren<EnemyAnimatorManager>();
        if (!enemyRigidbody2D) enemyRigidbody2D = GetComponent<Rigidbody2D>();
    ...
    void Start()
    {
        // Rigidbody2D recommended setup for top-down
        enemyRigidbody2D.gravityScale = 0f;
        enemyRigidbody2D.freezeRotation = true;
        enemyRigidbody2D.interpolation = RigidbodyInterpolation2D.Interpolate;
    }
    ...
    {
        Collider2D[] hits = Physics2D.OverlapCircleAll(transform.position, detectionRadius, playerDetectionLayer);

        // pick closest PlayerStats
    ...
    public void HandleDetection2()
    {

        Collider[] colliders = Physics.OverlapSphere(transform.position, detectionRadius, playerDetectionLayer);
    ...
    // ---------- LOCOMOTION (2D physics + animations) ----------
    public void HandleMoveToTarget()
    {
        Vector2 toTarget;
        //if (enemyManager.isPerformingAction || enemyManager.isInteracting) return;
        //if (currentTarget == null) { SetAnimMoving(false); return; }
        if (currentTarget != null  )
        {
         toTarget = (Vector2)(currentTarget.transform.position - transform.position);

        }
        else if(currentTarget == null && enemyManager.currentGateTarget)
        {
            toTarget = (Vector2)(enemyManager.currentGateTarget.transform.position - transform.position);
        ...
        if (dist > stoppingDistance)
        {
            ...
            Vector2 next = enemyRigidbody2D.position + dir * moveSpeed * Time.fixedDeltaTime;
            enemyRigidbody2D.MovePosition(next);
    ...
    public void SetAnimMoving(bool moving)
    {
        ...
        anim.SetFloat("Horizontal", 0f);
        anim.SetFloat("Vertical", moving ? 1f : 0f);   // (0,1)=walk, (0,0)=idle in your setup
    }
