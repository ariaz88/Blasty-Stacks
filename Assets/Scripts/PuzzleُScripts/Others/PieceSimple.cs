...
public class PieceSimple : MonoBehaviour
{
    ...
    private void Awake()
    {
        //if (pieceId == 0) { pieceId = NextId++; Registry[pieceId] = this; }
...
public class PieceSimple2 : MonoBehaviour
{
    // ===== Identity / registry =====
    private static int NextId = 1;
    private static readonly Dictionary<int, PieceSimple> Registry = new();
    ...
    // ===== Shape footprint =====
    [Header("Shape (grid offsets)")]
    [Tooltip("Offsets relative to the anchor cell. Must include (0,0).")]
    [SerializeField] private List<Vector2Int> shapeOffsets = new() { Vector2Int.zero };
    ...
    private void Awake()
    {
        ...
        // Ensure we stay on the board plane
        if (board)
            transform.position = board.SnapToPlane(transform.position);
    }
    ...
    private void Start()
    {
        ...
        // STRICT RULE:
        // Only the main anchor determines placement.
        // No child inspection, no collider logic, no offsets solving.
        Vector2Int anchor = initialAnchor;

        transform.position = board.CellCenterWorld(anchor);
    ...
    private void BuildFootprint(Vector2Int anchor, List<Vector2Int> outCells)
    {
        outCells.Clear();
        for (int i = 0; i < shapeOffsets.Count; i++)
            outCells.Add(anchor + shapeOffsets[i]);
    }

    /// <summary>
    /// Place (or slide) the piece onto the board.
    /// This ONLY:
    /// - Validates board occupancy
    /// - Snaps root to anchor cell center
    /// </summary>
    public bool TryPlace(Vector2Int anchor)
    {
        if (!board) return false;
        ...
        if (_isPlaced)
        {
            int dx = anchor.x - _anchor.x;
            int dy = anchor.y - _anchor.y;
        ...
        if (!board.AreCellsPlaceableForMover(_footprint, pieceId))
            return false;
        ...
        _lastOccupied.Clear();
        _lastOccupied.AddRange(_footprint);

        _anchor = anchor;
        _isPlaced = true;

        // 🔒 ROOT SNAP ONLY — NO CHILD MANIPULATION
        transform.position = board.CellCenterWorld(_anchor);
    ...
    {
        // Absolute guarantee: PieceSimple never changes colliders.
        foreach (var c in GetComponentsInChildren<Collider2D>())
        {
            if (c is BoxCollider2D b && b.size != Vector2.one)
            {
                Debug.LogError(
                    $"[PieceSimple] COLLIDER MODIFIED on {name} — size={b.size}",
                    this
                );
            }
        }
