...
public class LevelGameManager1 : MonoBehaviour
{
    public static LevelGameManager Instance { get; private set; }
    ...
    // Called when ENEMY gate dies  -> level WON
    private void OnEnemyGateDestroyed()
    {
        if (CurrentState != GameState.Playing)
            return;
        ...
        // Optional: freeze gameplay while WinPanel is shown
        GameplayPause.SetPaused(true);
        ...
        // Notify HomeManager (previously done in EnemyGateStats)
        HomeManager.NotifyStageWon(hpPercent);
    ...
    // Called when PLAYER gate dies  -> Revive or Lose
    private void OnPlayerGateDestroyed()
    {
        if (CurrentState != GameState.Playing)
            return;

        // Pause gameplay while we decide Revive / Lose
        GameplayPause.SetPaused(true);
        ...
        if (allowSingleRevivePerStage && hasRevivedThisStage)
        {
            ...
            if (revivePanel != null)
            {
                revivePanel.ShowLosePanel();   // we will expose this as public
            }
        }
        else
        {
            ...
            if (revivePanel != null)
            {
                revivePanel.ShowRevivePanel(); // we will expose this as public
            }
        }
    }
    ...
    public void NotifyReviveAccepted()
    {
        ...
        // RevivePanel already calls GameplayPause.SetPaused(false) after revive logic.
    }

    // Called from RevivePanel when user chooses No Thanks or runs out of time
    public void NotifyReviveDeclined()
    {
        CurrentState = GameState.Lost;
        // Gameplay remains paused, Lose panel stays visible.
    }

    // Optional if you ever reload the same stage
    public void ResetForNewStage()
    {
        CurrentState = GameState.Playing;
        hasRevivedThisStage = false;
