...
public class UnitStatsRuntime1
{
    public float attack, defense, maxHP;
    public float attackSpeed, moveSpeed, attackRange;
    public FighterType type;
    ...
    // Optional level growth (percent per level)
    public void ApplyLevelGrowth(int level, float atkPct, float hpPct, float movePct, float atkSpdPct)
    {
        int d = Mathf.Max(0, level - 1);
        float gA = Mathf.Pow(1f + atkPct, d);
        float gH = Mathf.Pow(1f + hpPct, d);
        ...
        moveSpeed *= gMv;
        attackSpeed *= gAS;
        // defense/range usually don’t scale by level; add if you want
    }
...
//public class UnitStatsRuntime
//{
//    // a mutable copy you modify at runtime
//    public float attack;
