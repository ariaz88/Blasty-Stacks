...
- **2026-09-25** — **Hero-upgrade juice from the onboarding reference (10.9–11.9 s, read at NATIVE 30 fps — the 15 fps pass had mis-read the stats flash as a sweep):** new self-bootstrapping `UnitUpgradeFx` plays on **every** successful upgrade (Arash's call): (A) gold light pillar + white flash + rays + 3 sparkles over the hero, (B) white line + solid diamond flare per stat → yellow streaks → slow yellow haze. Binds to `OnUnitUpgraded` and plays **one frame later** (`SetHeader` re-instantiates the hero and `Destroy` is deferred). Hero is a SpriteRenderer rig at order 0 → pillar on a nested canvas at **order 1**; haze had to move on top (slot plates hid it). `UnitDetailView` gets 6 getters. **Verified in EDIT mode** (additive MenuScene, real hero via `SetHeader`, 8-beat filmstrip + measurements: diamonds 0 u off glyph centres, line starts 8 u after level). **NOT tested in Play mode / via a real Upgrade tap.** Also still unexplained: one sandbox session showed the focus gate blocking with no tutorial running — booted without StarterScene managers, 12 s Awake→Start gap; likely harness, re-check.
